...
brickTopCollide:
	...
	mov	r0, #1			//Initialize a counter variable
	ldr	r1, =ballBottom		//Load ballTop base address
	ldr	ballY, [r1,#8]		//Load balls y value
	ldr	ballLeft, [r1]		//Load balls leftmost value
	ldr	ballRight, [r1,#4]	//Load balls right most value
	ldr	r5, =brick1		//Load bricks base address
...
$loop:
	...
	ldrh	r1, [r5, #10]		//Load y value of bottom of brick
	sub	r1, #35			//Bricks are 30 pixels high, 5 pixels 
	cmp	ballY, r1		//Check if in y range
	bgt	$loopTest		//If not in the range, check the next brick
...
brickLeftCollide:
	...
	ldr	ballX, [r1,#8]		//Load balls x value
...
.loop:
	...
	ldrh	r1, [r5, #8]		//Load leftmost value of brick
	cmp	ballX, r1		//Check if the right side of the ball is less than brick
	blt	.loopTest		//If so, collision impossible, check next brick
	
	add	r1, #5			//5 pixel wiggle room
	...
	add	r1, #30			//Bricks are 30 pixels tall
	cmp	ballBottom, r1		//If the ball bottom is less than top of the brick
	ble	.done			//A collision has occurred
...
for:
	...
	sub	r1, #5			//5 pixel wiggle room
	cmp	ballX, r1		//If the right side of the ball is in range for the right side of brick
